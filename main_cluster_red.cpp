...
int main(int argc, char *argv[]){
    ...
    std::string method;
    int flag_i=0, flag_c=0, flag_o=0 ,flag_m=0, flag_ri=0;
    bool complete=false;

    std::string inputFile, configurationFile, outputFile, inputFile2;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (std::string(argv[i]) == "-ri") {     //path for dataset with reduced dimension
            inputFile2 = argv[i + 1];
            flag_ri = 1;
        }
    }

    if(flag_i == 0 || flag_c == 0 || flag_o == 0 || flag_m == 0 || flag_ri == 0 || (method.compare("Classic") != 0 && method.compare("LSH") != 0 && method.compare("Hypercube") != 0)){
    ...
    if (method.compare("Classic") == 0){
        ...
        clusters = lloyds(k, centroids, new_ar);        //Lloyds algorithm to create clusters
        ...
        TableSize = new_ar.size()/8;
        initialize_hash_table(L,k_h, new_ar);

        const std::clock_t method_start = std::clock();
        clusters = rev_lsh(L, k_h, new_ar, centroids);       //reverse assignment with LSH
        duration_method = ( std::clock() - method_start ) / (double) CLOCKS_PER_SEC;
        empty_lsh();
        ...
        h_func(new_ar, d, h_vec);          //calculate d h functions for every point
        vector<vector<int>> f_vec; 
        f_vec.resize(number_of_points,vector<int>(d));
        f_func(new_ar, d, h_vec, f_vec);    //Calculate d f functions for every point
        vector<pair<string,int>> str;
        concat(d, new_ar, str, f_vec);      //Concatenate f functions to create a string
        initialize_ht_cube(d, str);

        const std::clock_t method_start = std::clock();
        clusters = rev_cube(d,new_ar, centroids,M,probes);       //reverse assignment with hypercube
        duration_method = ( std::clock() - method_start ) / (double) CLOCKS_PER_SEC;
        empty_cube();
    ...
    vector<double> s = silhouette_red(ar, clusters, method,new_ar);
    ...
    vector<vector<double>>().swap(new_ar);
